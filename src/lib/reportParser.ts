...
/**
 * SCQA narrative block structure
 */
...
/**
 * Extract the governing thought (first bold sentence or H1 content) from report markdown
 */
export function extractGoverningThought(markdown: string): string {
    if (!markdown) return "";

    // Try to extract from a "Governing Thought" or "Key Finding" header
    const headerMatch = markdown.match(
        /#{1,3}\s*(?:governing\s*thought|key\s*finding|executive\s*summary)[:\s]*(.*?)(?:\n|$)/i
    );
...
/**
 * Parse SCQA (Situation, Complication, Question, Answer) blocks from markdown
 */
export function parseSCQABlocks(markdown: string): SCQABlock[] {
    ...
    const pattern =
        /(?:situation|context)[:\s]*(.*?)(?:\n|$).*?(?:complication|challenge)[:\s]*(.*?)(?:\n|$).*?(?:question|inquiry)[:\s]*(.*?)(?:\n|$).*?(?:answer|resolution|recommendation)[:\s]*(.*?)(?:\n|$)/gis;

    let match: RegExpExecArray | null;
    while ((match = pattern.exec(markdown)) !== null) {
        blocks.push({
            situation: match[1]?.trim() ?? "",
            complication: match[2]?.trim() ?? "",
