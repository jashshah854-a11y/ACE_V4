...
const PIPELINE_STEPS = [
  ...
  { key: "validator", label: "Data Validation", icon: Shield },
  { key: "clustering", label: "Behavioral Clustering", icon: Users },
  { key: "regression", label: "Predictive Modeling", icon: TrendingUp },
...
const getStepIndex = (currentStage?: string, currentStep?: string): number => {
  if (!currentStage && !currentStep) return -1;

  // Prefer current_stage (user-facing name) over current_step (internal name)
  const searchTerm = (currentStage || currentStep || "").toLowerCase();

  const index = PIPELINE_STEPS.findIndex(
    (s) => searchTerm.includes(s.label.toLowerCase()) ||
           s.label.toLowerCase().includes(searchTerm) ||
           searchTerm.includes(s.key) ||
...
export function PipelineStatus({ runId, onComplete }: PipelineStatusProps) {
  ...
  const currentStepIndex = getStepIndex(state.current_stage, state.current_step);
  // Use backend-provided progress directly, with fallback calculation
    ...
    >
      ...
      <div className="space-y-2">
        {PIPELINE_STEPS.map((step, index) => {
          // Check if this stage is active based on current_stage matching
          const isActive = state.current_stage
            ? step.label === state.current_stage
            : index === currentStepIndex;

          // Check if completed using backend steps_completed array
          const isCompleted = state.steps_completed?.includes(step.key) ||
                             index < currentStepIndex ||
                             isComplete;
          const isPending = !isActive && !isCompleted && !isComplete;
