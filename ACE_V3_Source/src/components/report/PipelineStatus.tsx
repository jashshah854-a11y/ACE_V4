...
import { getRunState, RunState, RunStatus } from "@/lib/api-client";
...
interface PipelineStatusProps {
  runId: string;
  onComplete?: () => void;
  onStatusChange?: (status?: RunStatus, state?: RunState) => void;
}
...
export function PipelineStatus({ runId, onComplete, onStatusChange }: PipelineStatusProps) {
  ...
  useEffect(() => {
    ...
    if (prevStatus.current !== state.status) {
      ...
      onStatusChange?.(state.status as RunStatus, state);
    }
  ...
  const connectorLabel = sourceMeta?.connector ? sourceMeta.connector.replace(/_/g, " ") : "Manual upload";
  ...
  return (
    <TooltipProvider>
      ...
      >
        ...
        {/* Source Metadata */}
        <div className="mb-6 rounded-xl border border-border/40 bg-muted/40 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-muted-foreground">Source</p>
              <p className="font-semibold capitalize">{connectorLabel}</p>
            </div>
            <Badge variant="outline" className="text-xs">
              {sourceMeta?.connector ? "Automated" : "Manual"}
            </Badge>
          </div>
          <div className="grid grid-cols-2 gap-3 mt-3 text-sm">
            <div>
              <p className="text-xs text-muted-foreground">Created</p>
              <p className="font-medium">{formatDate(state.created_at || state.started_at)}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Status</p>
              <p className="font-medium capitalize">{state.status || "unknown"}</p>
            </div>
          </div>
          {sourceMeta && Object.entries(sourceMeta).filter(([key]) => key !== "connector").length > 0 && (
            <dl className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
              {Object.entries(sourceMeta)
                ...
                .map(([key, value]) => (
                  <div key={key}>
                    <dt className="uppercase tracking-wide text-[10px] text-muted-foreground/70">{key.replace(/_/g, " ")}</dt>
                    <dd className="text-[11px] text-foreground truncate">{String(value)}</dd>
                  </div>
                ))}
            </dl>
          )}
