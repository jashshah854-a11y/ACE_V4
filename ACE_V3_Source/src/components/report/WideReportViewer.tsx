...
import { Copy, FileDown, ShieldAlert, Activity, BarChart3 } from "lucide-react";
import {
    extractMetrics,
...
import { SourceMetadata } from "../../lib/api-client";
...
export function WideReportViewer({
    ...
    const { data: diagnostics, loading: diagnosticsLoading, error: diagnosticsError } = useDiagnostics(runId);
    const { data: modelArtifacts, loading: artifactsLoading, error: artifactsError } = useModelArtifacts(runId);
    ...
    const normalizedConnector = sourceMeta?.connector
        ? sourceMeta.connector.replace(/_/g, " ")
        : undefined;
    const normalizedStatus = runStatus ? runStatus.replace(/_/g, " ") : undefined;
    ...
    const featureImportance = useMemo(() => {
        const raw = (modelArtifacts as any)?.feature_importance;
        if (!raw) return [] as { name: string; value: number }[];
        const normalized: { name: string; value: number }[] = [];
        if (Array.isArray(raw)) {
            raw.slice(0, 5).forEach((item: any, index: number) => {
                const name = item?.name || item?.feature || `feature_${index + 1}`;
                const value = typeof item === "number" ? item : Number(item?.value ?? item?.importance ?? 0);
        ...
        if (typeof raw === "object") {
            Object.entries(raw)
                .slice(0, 5)
                .forEach(([key, value]) => {
                    const numeric = typeof value === "number" ? value : Number((value as any)?.importance ?? 0);
                    normalized.push({ name: key, value: numeric });
                });
            return normalized;
        }
        return normalized;
    ...
    const renderSourceSummary = () => {
        if (!runId && !sourceMeta && !createdAt) return null;
        return (
            <Card className="mb-4 p-4 space-y-3">
                <div className="flex items-center justify-between">
                    <div>
                        <div className="text-xs uppercase text-muted-foreground">Data Source</div>
                        <div className="text-sm font-semibold capitalize">{normalizedConnector || "Manual upload"}</div>
                    </div>
                    {runStatus && (
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                        ...
                        <p className="font-mono text-xs bg-muted/40 px-1 py-0.5 rounded">
                ...
                {sourceEntries.length > 0 && (
                    <dl className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                        {sourceEntries.map(([key, value]) => (
                            <div key={key}>
                                <dt className="uppercase tracking-wide text-[10px]">{key.replace(/_/g, " ")}</dt>
        ...
        <Card className="mb-4 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                    ...
                    <p className="font-medium capitalize">{diagnostics?.mode || (safeMode ? "safe" : "standard")}</p>
                </div>
                <div>
                    <p className="text-muted-foreground">Confidence</p>
                    <p className="font-medium">
                        {typeof diagnostics?.confidence?.data_confidence === "number"
                            ? `${diagnostics.confidence.data_confidence.toFixed(1)}%`
                            : "—"}
                    </p>
                </div>
            </div>
            ...
            {diagnostics?.validation?.failed_fields?.length ? (
                <div className="text-xs text-muted-foreground">
                    Validation gaps: {diagnostics.validation.failed_fields.slice(0, 3).join(", " )}
                </div>
            ) : null}
            {diagnostics?.identity?.missing_fields?.length ? (
                <div className="text-xs text-muted-foreground">
                    Missing fields: {diagnostics.identity.missing_fields.slice(0, 3).join(", " )}
                </div>
            ) : null}
        </Card>
    );
    ...
    const renderModelArtifactsCard = () => {
        if (!runId) return null;
        if (!artifactsLoading && !artifactsError && featureImportance.length === 0) return null;
        return (
            <Card className="mb-4 p-4 space-y-3">
                <div className="flex items-center justify-between">
                    <div>
                        <div className="text-xs uppercase text-muted-foreground">Model Insights</div>
                        <div className="text-sm font-semibold">Top Predictive Signals</div>
                    </div>
                ...
                {artifactsError && (
                    <p className="text-xs text-destructive">{artifactsError}</p>
                )}
                {!artifactsLoading && !artifactsError && featureImportance.length > 0 && (
                    <div className="space-y-2">
                        {featureImportance.map((item, index) => (
                            <div key={`${item.name}-${index}`} className="flex items-center justify-between text-sm">
                                <span className="truncate">{item.name}</span>
                                <span className="font-mono text-xs">{typeof item.value === 'number' ? item.value.toFixed(2) : item.value}</span>
                            </div>
                        ))}
                    </div>
                )}
                {!artifactsLoading && !artifactsError && featureImportance.length === 0 && (
                    <p className="text-xs text-muted-foreground">No feature importance available.</p>
                )}
            </Card>
        );
    };
