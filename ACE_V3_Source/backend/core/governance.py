...
def rebuild_governance_artifacts(state_manager: StateManager) -> Dict[str, Dict]:
    ...
    cache_root = Path("data/cache/governance")
    cache_root.mkdir(parents=True, exist_ok=True)
    cache_key = ingestion_meta.get("sha256") if isinstance(ingestion_meta, dict) else None
    cache_path = cache_root / f"{cache_key}.json" if cache_key else None
    cached = None
    ...
    confidence = None
    if cached:
        identity_card = cached.get("identity_card")
        task_contract = cached.get("task_contract")
        confidence = cached.get("confidence_report")
