...
def _intake_only_enabled(run_config, state):
    cfg = run_config or {}
    flag = cfg.get('intake_only')
    if flag is None:
        stored = state.read('run_config') or {}
        flag = stored.get('intake_only')
    if flag is None:
        flag = os.getenv('ACE_INTAKE_ONLY') not in {None, '', '0', 'false', 'False'}
    return bool(flag)
...
def run_ace_v3(data_path, run_config=None):
    ...
    try:
        ...
        cache_key = _anomaly_cache_key(intake_result)
        cached_clean = _load_anomaly_cache(cache_key, state)
        if cached_clean:
            data_path = str(cached_clean)
            log_ok(f"Anomaly cache hit. Cleaned dataset: {data_path}")
        else:
            try:
                ...
                _store_anomaly_cache(cache_key, anomaly_result, cleaned_path)
    ...
    if _intake_only_enabled(run_config, state):
        log_info('Intake-only mode enabled; skipping downstream agents.')
