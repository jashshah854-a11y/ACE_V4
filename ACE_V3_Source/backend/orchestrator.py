...
from observability.metrics import inc_counter, record_job_duration, set_gauge, write_metrics_file
...
def orchestrate_new_run(data_path, run_config=None, run_id=None):
    ...
    log_event(ORCH_LOGGER, "info", "run_start", data_path=data_path, run_id=run_id)
    ...
    if not os.path.exists(data_path):
        ...
        log_event(ORCH_LOGGER, "error", "data_file_missing", data_path=data_path)
        shutil.rmtree(run_path, ignore_errors=True)
        inc_counter("orchestrator_runs_total", {"status": "file_missing"})
        write_metrics_file()
        return None, None
    ...
    try:
        ...
        log_event(ORCH_LOGGER, "error", "ingestion_failed", error=str(e))
        shutil.rmtree(run_path, ignore_errors=True)
        inc_counter("orchestrator_runs_total", {"status": "ingestion_failed"})
        write_metrics_file()
        return None, None
    ...
    if run_config:
        ...
        source_meta = run_config.get("source") if isinstance(run_config, dict) else None
        if source_meta:
            state["source"] = source_meta
            try:
                state_manager.write("source_metadata", source_meta)
            except Exception as exc:
                print(f"[WARN] Unable to persist source metadata: {exc}")
    ...
    save_state(state_path, state)
    log_event(ORCH_LOGGER, "info", "run_initialized", run_id=run_id, run_path=str(run_path))
    inc_counter("orchestrator_runs_total", {"status": "initialized"})
    set_gauge("orchestrator_last_run_epoch", time.time())
    record_job_duration("orchestrator_initialization_seconds", start_time, {"run_id": str(run_id)})
    write_metrics_file()
...
def main_loop(run_path):
    ...
    while True:
        ...
        if current == "ingestion":
            step_state = state["steps"].setdefault("ingestion", {"name": "ingestion"})
            if step_state.get("status") != "completed":
                step_state["status"] = "completed"
                step_state["completed_at"] = iso_now()
                state["steps"]["ingestion"] = step_state
                if "ingestion" not in state.get("steps_completed", []):
                    state.setdefault("steps_completed", []).append("ingestion")
                update_history(state, "ingestion completed")
