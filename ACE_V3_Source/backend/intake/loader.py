...
class IntakeLoader:
    ...
    def _load_file(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        if ext in [".csv", ".tsv"]:
            sep = "\t" if ext == ".tsv" else ","
            try:
                table_meta = self._save_streaming_csv(file_path, sep)
            except Exception as e:
                print(f"?s??,? CSV Load Error (Retrying with python engine): {e}")
                table_meta = self._save_streaming_csv(file_path, sep, engine='python')
    ...
    def _save_streaming_csv(self, file_path: Path, sep: str, engine: str | None = None) -> Dict[str, Any]:
        safe_name = "".join([c if c.isalnum() else "_" for c in file_path.stem]).lower()
        save_path = self.tables_dir / f"{safe_name}.csv"
        ...
        mode = "w"
        chunks = pd.read_csv(file_path, sep=sep, on_bad_lines="warn", chunksize=200000, engine=engine)
        for chunk in chunks:
            if columns is None:
                columns = list(chunk.columns)
                sample_columns = columns[:5]
            row_count += len(chunk)
            chunk.to_csv(save_path, index=False, mode=mode, header=(mode == "w"))
            mode = "a"
        if columns is None:
            columns = []
            sample_columns = []
